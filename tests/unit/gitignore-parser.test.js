...
describe('GitignoreParser', () => {
  ...
  describe('parseGitignore', () => {
    ...
    test('should handle negated patterns correctly', () => {
      ...
      expect(result.includePatterns).toContainEqual(expect.stringMatching(/node_modules\/important-package\//));
    });
  ...
  describe('_parseGitignoreContent', () => {
    test('should skip empty lines and comments', () => {
      ...
      const nonBuildPatterns = result.excludePatterns.filter(p => 
        !p.includes('bundle.js') && 
        !p.includes('index.js.map') && 
        !p.includes('output.css')
      );
